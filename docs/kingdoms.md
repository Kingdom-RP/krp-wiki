...
!!! note "Настройка"
    Множители задаются в серверном конфиге (`kingdomrpcore-server.toml`, секция `[upkeep]`:
    `foodPerResident`, `materialsPerChunk`, `prosperityPerLevel`) — админ может ребалансить.
